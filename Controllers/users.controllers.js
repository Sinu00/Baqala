...
const homepage = async (req, res) => {
  try {
    ...
    for (let category of categories) {
      category.products = await Product.find({ category: category.name, status: "active" });
    }
    res.render('pages/home', { products, categories,cartLength,banners ,bannerTitle, featuredTitle ,userId });
  } catch (err) {
    console.log(err);
  }
};
